...
async def graceful_shutdown():
    ...
    while wait_time < MAX_WAIT_SECONDS:
        connection_count = await redis_pubsub.get_connection_count()
        if connection_count == 0:
            logging.info("All WebSocket clients disconnected. Shutting down.")
            return
        remaining = MAX_WAIT_SECONDS - wait_time
    ...
    connections = await redis_pubsub.get_connection_count()
...
async def publish_shutdown():
    locked = await redis_pubsub.redis.setnx("shutdown_lock", "1")
    if locked:
